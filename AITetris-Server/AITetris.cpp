...
ID3DXFont*              g_pMiddleFont = NULL;         // Font for drawing text
ID3DXSprite*            g_pTextSprite = NULL;   // Sprite for batching draw text calls
CDXUTTextHelper*		g_txtHelper=NULL;
CDXUTTextHelper*		g_middleTxtHelper=NULL;
...
INT WINAPI WinMain( HINSTANCE, HINSTANCE, LPSTR, int )
{
    ...
    DXUTCreateWindow( L"AI Tetris for CS202" );		
...
/*	DWORD dwWindowStyle = WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU | WS_THICKFRAME | 
	...
	DXUTAdjustWindowStyle( DXUTGetHWND(), true);*/
	...
	TextOut(hdc,460,400,L"잠시만 기다리셈....Wait....",20);
	ReleaseDC(DXUTGetHWND(),hdc);


	
	if(!game.bInitializeSoundSystem(DXUTGetHWND()))
	{
		MessageBox(NULL,L"사운드 초기화에 실패하였습니다. 소리가 안나와요ㅡㅡ",L"Sound initialization failed",MB_OK);

	}
	srand(GetTickCount());//(NULL));
	
	SetTimer(DXUTGetHWND(),1,1,NULL); //게임 처리를 위한 타이머 설치 TIMER ID-1
...
void InitApp()
{
    ...
    g_SampleUI.AddEditBox( 20, L"Test", 35, iY += 24, 125, 50 );
*/
...
HRESULT CALLBACK OnCreateDevice( IDirect3DDevice9* pd3dDevice, const D3DSURFACE_DESC* pBackBufferSurfaceDesc )
{
    ...
    WCHAR str[MAX_PATH];
    V_RETURN( DXUTFindDXSDKMediaFileCch( str, MAX_PATH, L"AITetris.fx" ) );
    ...
    //return S_OK;
	return game.OnCreateDevice(pd3dDevice);   
}
                                ...
                                const D3DSURFACE_DESC* pBackBufferSurfaceDesc )
{
	...
	if( g_pMiddleFont )
        V_RETURN( g_pMiddleFont->OnResetDevice() );
	...
	g_txtHelper=new CDXUTTextHelper( g_pFont, g_pTextSprite, 15 );	
	g_middleTxtHelper=new CDXUTTextHelper( g_pMiddleFont, g_pTextSprite, 20 );	
    return S_OK;
}
...
void CALLBACK OnFrameRender( IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime )
{
    ...
    // Clear the render target and the zbuffer 
	 ...
	 V( pd3dDevice->Clear(0, NULL, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER, D3DCOLOR_ARGB(0, 0,0,0), 1.0f, 0) );//45, 50, 170
    ...
    if( SUCCEEDED( pd3dDevice->BeginScene() ) )
    {
		...
		game.render(pd3dDevice,fElapsedTime);		
		RenderText();
...
void RenderText()
{
    ...
    g_txtHelper->DrawTextLine( DXUTGetFrameStats() );
    g_txtHelper->DrawTextLine( DXUTGetDeviceStats() );

/*    g_txtHelper->SetForegroundColor( D3DXCOLOR( 1.0f, 1.0f, 1.0f, 1.0f ) );
	WCHAR data[1000];
	swprintf(data,L"%d",sizeof(WCHAR));
    g_txtHelper->DrawTextLine( data );*/
	//L"Put whatever misc status here"
    
    // Draw help
    if( g_bShowHelp )
    {
        g_txtHelper->SetInsertionPos( 10, pd3dsdBackBuffer->Height-15*6 );
        g_txtHelper->SetForegroundColor( D3DXCOLOR( 1.0f, 0.75f, 0.0f, 1.0f ) );
        g_txtHelper->DrawTextLine( L"Controls (F1 to hide):" );

        g_txtHelper->SetInsertionPos( 40, pd3dsdBackBuffer->Height-15*5 );
        g_txtHelper->DrawTextLine( L"Blah: X\n"
                                L"Quit: ESC" );
    }
    else
    {
        g_txtHelper->SetInsertionPos( 10, pd3dsdBackBuffer->Height-15*2 );
        g_txtHelper->SetForegroundColor( D3DXCOLOR( 1.0f, 1.0f, 1.0f, 1.0f ) );
        g_txtHelper->DrawTextLine( L"by Pureku. Ver 0.9.5" );
    }
    g_txtHelper->End();
}
...
LRESULT CALLBACK MsgProc( HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam, bool* pbNoFurtherProcessing )
{
    ...
    *pbNoFurtherProcessing = g_SampleUI.MsgProc( hWnd, uMsg, wParam, lParam );
	...
	game.OnMsgProc(hWnd,uMsg,wParam,lParam,pbNoFurtherProcessing);
...
void CALLBACK OnLostDevice()
{
	...
	if(g_txtHelper)
		delete g_txtHelper;
	if(g_middleTxtHelper)
		delete g_middleTxtHelper;
	g_txtHelper=NULL;
	g_middleTxtHelper=NULL;
}
